...
if [ -n "${validation_merge_list}" ];then
	...
	base_register_merge_file_list=$(fd . "${BUCK_UP_DIR_PATH}" -d 6 -t f | rga "/${BUCK_UP_DIR_NAME}/[0-9]{4}/[0-9]{2}/[0-9]{2}/[0-9]{4}/${BUCKUP_MERGE_CONTENSTS_LIST_DIR_NAME}/${BUCKUP_MERGE_CONTENSTS_LIST_FILE_NAME}" | sort -r | gsed -r "s/^(${SED_BUCK_UP_DIR_PATH}\/[0-9]{4}\/[0-9]{2}\/[0-9]{2})(\/[0-9]{4}\/${BUCKUP_MERGE_CONTENSTS_LIST_DIR_NAME}\/${BUCKUP_MERGE_CONTENSTS_LIST_FILE_NAME})/\2\t\1/" | uniq -f 1  | gsed -r "s/^(\/[0-9]{4}\/${BUCKUP_MERGE_CONTENSTS_LIST_DIR_NAME}\/${BUCKUP_MERGE_CONTENSTS_LIST_FILE_NAME})\t(${SED_BUCK_UP_DIR_PATH}\/[0-9]{4}\/[0-9]{2}\/[0-9]{2})/\2\1/" | head -n ${base_register_date_order})
else
	...
	base_register_merge_file_list=$(fd . "${BUCK_UP_DIR_PATH}" -d 6 -t f | rga "/${BUCK_UP_DIR_NAME}/[0-9]{4}/[0-9]{2}/[0-9]{2}/[0-9]{4}/${BUCKUP_MERGE_CONTENSTS_LIST_DIR_NAME}/${BUCKUP_MERGE_CONTENSTS_LIST_FILE_NAME}" | sort | gsed -r "s/^(${SED_BUCK_UP_DIR_PATH}\/[0-9]{4}\/[0-9]{2}\/[0-9]{2})(\/[0-9]{4}\/${BUCKUP_MERGE_CONTENSTS_LIST_DIR_NAME}\/${BUCKUP_MERGE_CONTENSTS_LIST_FILE_NAME})/\2\t\1/" | uniq -f 1 | gsed -r "s/^(\/[0-9]{4}\/${BUCKUP_MERGE_CONTENSTS_LIST_DIR_NAME}\/${BUCKUP_MERGE_CONTENSTS_LIST_FILE_NAME})\t(${SED_BUCK_UP_DIR_PATH}\/[0-9]{4}\/[0-9]{2}\/[0-9]{2})/\2\1/" | sort -r | head -n ${base_register_date_order} | sort )
fi
# ------------------------------------------------------------------------------------
...
base_list_source=$(gzcat ${base_register_merge_list_path})
...
day_depth_path_list=$(fd  -t d --exact-depth 3 . ${BUCK_UP_DIR_PATH}  | sort)
...
if [ -n "${validation_merge_list}" ]; then
	delete_list=$(join -v 2  <(echo "${buck_up_data_list}" | gsed -e 's/'${DFBK_GGIP_EXETEND}'//' -e 's/\ /'${DIFBK_BLANK_MARK}'/g' | sort ) <(echo "${base_list_cont}" | sed 's/\ /'${DIFBK_BLANK_MARK}'/g' | sort) | sed 's/^/99999\t/g' | sed 's/'${DIFBK_BLANK_MARK}'/\ /g')
	...
	if [ -n "${delete_list}" ];then
		...
		modify_merge_list=$(cat <(echo "${delete_list}" | gsed -r "s/^([0-9d]{1,20})\t/${delete_list_mark}\t/") <(echo "${modify_merge_list}") | sort -k 2,2 | uniq -u -f 1 | rga -v "^${delete_list_mark}\t")
	fi
...
echo "${clean_list}" | sed 's/^/'${SED_TARGET_PAR_DIR_PATH}'/g' | gxargs -P ${ENV_PARARELL_EXEMNUM} -d'\n'  -I{} bash -c "exec_delete_gzip \"{}\"" | pv
...
do
	...
	echo "${empty_dir_list}" | gxargs  -P ${ENV_PARARELL_EXEMNUM} -d'\n'  -I{} bash -c "exec_delete_gzip \"{}\""
done
...
do
	...
	echo "${top_empty_dir_list}" | gxargs -P ${ENV_PARARELL_EXEMNUM} -d'\n'  -I{} rm -rf "{}"
done
echo "clean complete !"
