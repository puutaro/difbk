...
make_desk_rga_comd(){
	...
	*)
		case "${2}" in
			"")
				local desk_rga_cmd=$(echo ${D_OPTION} | gsed -re "s/(${DESC_PREFIX})/\" | rga -A ${1} ${dRGA_OPTION} \"\1/g" -e 's/^"//' -e 's/$/"/' )
				;;
			*)
				local desk_rga_cmd=$(echo ${D_OPTION} | gsed -re "s/(${DESC_PREFIX})/\" | rga -A ${1} ${dRGA_OPTION} \"\\\[2\\\] \1/g" -e 's/^"//' -e 's/$/"/' )
				;;
		esac
		;;
	esac
...
make_desk_rga_v_comd(){
	...
	*)
		case "${2}" in
			"")
				local desk_rga_v_cmd=$(echo ${DV_OPTION} | gsed -re "s/,(${DESC_PREFIX})\.\*([^,]{1,100})/ | gsed '\/\1.*\2\/,+${1}d'/g")
				;;
			*)
				local desk_rga_v_cmd=$(echo ${DV_OPTION} | gsed -re "s/,(${DESC_PREFIX})\.\*([^,]{1,100})/ | gsed '\/\\\[2\\\] \1.*\2\/,+${1}d'/g")
		esac
		;;
	esac
	echo "${desk_rga_v_cmd}"
