...
case "${get_buckup_file_path}" in 
	...
	*)
		...
		source_file_deflost_path="${DFBK_RESTORE_DIR_PATH}/$(echo "${get_buckup_file_path}" | gsed -e 's/\.\.//' -re "s/([^a-zA-Z0-9_-])/_/g")"
		merge_cat_path="${DFBK_RESTORE_DIR_PATH}/merge_contents.txt"
		gzcat "${source_file_path}" > "${source_file_deflost_path}"
		# set target file path
		case "${rs_arg_list[1]}" in 
			"")
				target_file_path=$(echo "${get_buckup_file_path}" | gsed -e "s/^${SED_BUCK_UP_DIR_PATH}\/[0-9]\{4\}\/[0-9]\{2\}\/[0-9]\{2\}\/[0-9]\{4\}\/${BACKUP_CREATE_DIR_NAME}\/${TARGET_DIR_NAME}\///g"  -e "s/^\.\.\/${BUCK_UP_DIR_NAME}\/[0-9]\{4\}\/[0-9]\{2\}\/[0-9]\{2\}\/[0-9]\{4\}\/${BACKUP_CREATE_DIR_NAME}\/${TARGET_DIR_NAME}\///" -e 's/'${SED_DFBK_GGIP_EXETEND}'//')
				;;
		...
		case "${C_OPTION}" in 
			"")
				...
				wait; rm "${source_file_deflost_path}" "${merge_cat_path}"
				;;
			-c) 
				case "${rs_arg_list[1]}" in 
					"")
...
if [ -n "$(echo "${rs_arg_list[2]}")" ];then  
	grep_path=$(echo "${rs_arg_list[2]}" | sed 's/^'${SED_TARGET_PAR_DIR_PATH}'//g' | sed 's/^\///g')
	# lecho "rs:0: ### grep_path: ${grep_path}"
	ls_buckup_merge_contents=$(gzcat "${rs_arg_list[0]}" | rga "${BUCK_UP_DIR_NAME}/[0-9]{4}/[0-9]{2}/[0-9]{2}/[0-9]{4}/${BACKUP_CREATE_DIR_NAME}/${TARGET_DIR_NAME}/${grep_path}" || e=$?);
	...
	ls_buckup_merge_contents=$(gzcat "${rs_arg_list[0]}");
fi
...
# ls ../old_monorepo/2022/06/02/1047/buckup_merge/buckup_merge_list.txt.difbk.gz
while [ true ]
do
	rs_tar_insert_dir_path=$(echo "${restore_target_path}/${TARGET_DIR_NAME}" | gsed -e 's/\/\//\//g' -e 's/\/$//')
