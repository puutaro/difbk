...
mklabel="-mklabel"
mklabel_con=$(echo "${GENERAL_OPTION_LIST_CON}" | rga "\\${mklabel}" | sed "s/${mklabel}//" | sed "s/^\s*//" | sed "s/\s*$//" | sed -r "s/'(.*)'$/\1/" | rga -v '\\' | rga -v '["\]\[\?!]')
lslabel="-lslabel"
lslabel_con=$(echo "${GENERAL_OPTION_LIST_CON}" | rga "\\${lslabel}")
rmlabel="-rmlabel"
rmlabel_con=$(echo "${GENERAL_OPTION_LIST_CON}" | rga "\\${rmlabel}" | sed "s/${rmlabel}//" | sed "s/^\s*//" | sed "s/\s*$//" | sed -r "s/'(.*)'$/\1/")
label_option_exec=0
...
case "${mklabel_con}" in 
	...
	*) 
		if [ ! -e "${DFBK_LABEL_FILE_PATH}" ] || [ ! -s "${DFBK_LABEL_FILE_PATH}" ];then 
			...
			mklabel_con=$(echo "${mklabel_con}" | sed -r 's/([^a-zA-Z0-9_])/\\\1/g' )
			sed "1i ${mklabel_con}" -i "${DFBK_LABEL_FILE_PATH}"
		fi
	;;
esac
case "${rmlabel_con}" in 
	"");; *) 
			sed_rmlabel_con=$(echo "${rmlabel_con}" | sed -r 's/([\\])/\\\\\\\1/g' | sed -r "s/([^a-zA-Z0-9_'\#\)\(\|\{\}\+\;\:\<\>])/\\\\\1/g")
			rm_after_con=$(cat "${DFBK_LABEL_FILE_PATH}" | sed "s/^${sed_rmlabel_con}$//g" | sed '/^$/d') 
...
eval "fd -IH $(cat "${DFBK_IGNORE_FILE_PATH}" | rga -v "^#" | sed -e 's/\[/\\\[/g' -e 's/\]/\\\]/g' -re "s/^([^ ])/\/\1/" -re "s/([^ ])$/\1/" -e 's/\/\//\//' -re 's/^([^ ])/ -E\ \1/' | tr -d '\n') . \"${TARGET_DIR_PATH}\"" | rga -v "/${DFBK_SETTING_DIR_NAME}" | sed "1i ${TARGET_DIR_PATH}" | rga -v "${DFBK_IGNORE_FILE_NAME}$" | rga -v "${DIFBK_EXCLUDE_DS_STORE}$" > ${DFBK_CUR_FD_CON_FILE_PATH} && 
difbk_sum_dummy_file_name="check_sum_dummy_file"
cat "${DFBK_CUR_FD_CON_FILE_PATH}" | sed -e '/^$/d' -e "s/^/\"/" -e "s/$/\" \\\\/" -e '1i sum \\' | awk -v DFBK_CHECK_SUM_CULC_DIR_PATH="${DFBK_CHECK_SUM_CULC_DIR_PATH}"  '(NR%50==0){$0=$0"\n""> ""\042"DFBK_CHECK_SUM_CULC_DIR_PATH"/ch_file_"int(rand() * 100000)int(rand() * 100000)int(rand() * 100000)int(rand() * 100000)int(rand() * 100000)int(rand() * 100000)"\042"" &""\nsum ""\\"}{print}' | sed -e "$ s/\\\\$/\"${difbk_sum_dummy_file_name}\" > \"${SED_DFBK_CHECK_SUM_CULC_DIR_PATH}\/ch_file_${RANDOM}${RANDOM}${RANDOM}${RANDOM}${RANDOM}${RANDOM}\" \&/" -e '$ a wait' > "${DFBK_EXEC_SUM_FILE_PATH}" && LANG=C bash "${DFBK_EXEC_SUM_FILE_PATH}" 2>/dev/null && fd -t f . "${DFBK_CHECK_SUM_CULC_DIR_PATH}" | sed -e '/^$/d' -e "s/^/\"/" -e "s/$/\" \\\\/" -e '1i cat \\' | awk -v DFBK_CHECK_SUM_OUTPUT_FILE_RCEIVE_PATH=${DFBK_CHECK_SUM_OUTPUT_FILE_RCEIVE_PATH} '(NR%200==0){$0=$0"\n"">> ""\042"DFBK_CHECK_SUM_OUTPUT_FILE_RCEIVE_PATH"\042""\ncat ""\\"}{print}' | sed "$ s/\\\\$/ >> \"${SED_DFBK_CHECK_SUM_OUTPUT_FILE_RCEIVE_PATH}\"/" > "${DFBK_CHECK_SUM_OUTPUT_FILE_PATH}" && bash "${DFBK_CHECK_SUM_OUTPUT_FILE_PATH}" &
# -----------------------------------------------------------------------------
#list check if no list, fact dire search -------------------------
if [  "${BUCK_UP_DIR_PATH}" ];then
...
case "${merge_list_file_path}" in 
	...
	*)
		if [ -e "${merge_list_file_path}" ];then 
			LS_BACKUP_DIR_CONTENTS=$(zcat "${merge_list_file_path}" | sed '/^$/d')
			row_num_merge_list_file=$(echo "${LS_BACKUP_DIR_CONTENTS}" | wc -l)
...
case "${edit_desk_code}" in 
	"")
		...
		case "${edit_desk_contents}" in 
			...
			*) 	
				if [ -n "${stamp_label}" ] ;then 
					sed_stamp_label=$(echo "${stamp_label}" | sed -r 's/([^a-zA-Z0-9_])/\\\1/g')
		...
		if [ ! -e ${BUCKUP_DESC_TEMP_FILE_PATH} ] || [ -z "$(cat "${BUCKUP_DESC_TEMP_FILE_PATH}" | sed 's/\s//g' | sed '/^$/d')" ]; then exit 0 ;fi
...
ls_current_dir_contents=$(cat <(join -v 1 <(cat "${DFBK_CUR_FD_CON_FILE_PATH}" | sed -e 's/\ /'${DIFBK_BLANK_MARK}'/g' -e 's/\ /'${DIFBK_BLANK_MARK}'/g' | sort) <(cat "${DFBK_CHECK_SUM_OUTPUT_FILE_RCEIVE_PATH}"  | sed -re 's/^([0-9]{1,100})\ */\1\t/' -e 's/\ /\t/' | cut -f3 | sed 's/\ /'${DIFBK_BLANK_MARK}'/g' | sort) | sed -e 's/'${DIFBK_BLANK_MARK}'/\ /g' -e 's/^/'${CHECH_SUM_DIR_INFO}'\t/') <(cat "${DFBK_CHECK_SUM_OUTPUT_FILE_RCEIVE_PATH}"  | sed -re 's/^([0-9]{1,100})\ */\1\t/' -e 's/\ /\t/' | cut -f1,3) | sed 's/'${SED_TARGET_PAR_DIR_PATH}'//g' | sort -k 2,2)

# cat "${DFBK_CHECK_SUM_OUTPUT_FILE_RCEIVE_PATH}"  | sed -r 's/^([0-9]{1,100})\ */\1\t/' | sed 's/\ /\t/' | cut -f1,3
rm -rf "${DFBK_CHECK_SUM_CULC_DIR_PATH}" && 
mkdir -p "${DFBK_CHECK_SUM_CULC_DIR_PATH}" &
rm -rf  "${DFBK_CHECK_SUM_CULC_DIR_PATH}" \
...
case "${ls_delete_buckup_merge_contents}" in "");; 
	*)
		...
		ls_buckup_merge_contents=$(cat <(echo "${ls_delete_buckup_merge_contents}" | sed -r "s/^([0-9d]{1,20})\t/\1\t${delete_list_mark}\t/") <(echo "${ls_buckup_merge_contents}" | sed -r "s/^([0-9d]{1,20})\t(\/${BUCK_UP_DIR_NAME}\/[0-9]{4}\/[0-9]{2}\/[0-9]{2}\/[0-9]{4}\/${BACKUP_CREATE_DIR_NAME})(.*)/\1\t\2\t\3/") | sort -k 3 | uniq -u -f 2 | rga -v "[0-9d]{1,20}\t${delete_list_mark}\t" | sed -r "s/^([0-9d]{1,20})\t(\/${BUCK_UP_DIR_NAME}\/[0-9]{4}\/[0-9]{2}\/[0-9]{2}\/[0-9]{4}\/${BACKUP_CREATE_DIR_NAME})\t(.*)/\1\t\2\3/")
	;;
esac
...
case "${ls_create_buckup_merge_contents}" in "");; 
	*)
		...
		ls_buckup_merge_contents=$(cat <(echo "${ls_create_buckup_merge_contents}" | sed -r "s/^([0-9d]{1,20})\t/\1\t${create_list_mark}\t/") <(echo "${ls_buckup_merge_contents}" | sed -r "s/^([0-9d]{1,20})\t(\/${BUCK_UP_DIR_NAME}\/[0-9]{4}\/[0-9]{2}\/[0-9]{2}\/[0-9]{4}\/${BACKUP_CREATE_DIR_NAME})(.*)/\1\t\2\t\3/") | sort -k 3 | uniq -u -f 2 | rga -v "[0-9d]{1,20}\t${create_list_mark}\t" | sed -r "s/^([0-9d]{1,20})\t(\/${BUCK_UP_DIR_NAME}\/[0-9]{4}\/[0-9]{2}\/[0-9]{2}\/[0-9]{4}\/${BACKUP_CREATE_DIR_NAME})\t(.*)/\1\t\2\3/")
		;;
esac
# ------------------------------------------------------------------------
...
case "${after_desc_contents}" in "");;
	*)
		time_prefix=$(echo "${BUCKUP_DESC_FILE_PATH}" | sed -e 's/'${SED_BUCK_UP_DIR_PATH}'\///' -e 's/\/'${BUCKUP_DESC_FILE_NAME}'//' -e 's/\//\\\//g')
...
case "${ls_delete_buckup_merge_contents}" in 
	"") delete_item_total=0 ;; 
	*) delete_item_total=$(echo "${ls_delete_buckup_merge_contents}" | wc -l)
		;;
esac
...
case "${ls_create_buckup_merge_contents}" in 
	"") create_item_total=0 ;; 
	*) create_item_total=$(echo "${ls_create_buckup_merge_contents}" | wc -l)
		;;
esac
echo "create_items total: ${create_item_total}"
echo "$(echo "${ls_create_buckup_merge_contents}" | cut -f2 | head -n ${DISPLAY_NUM_LIST})"
# --------------------------------------------------------------------
exit 0
